...
//use std::io;
use std::{env, fs, io::Error, io::ErrorKind, io::stdin};
...
fn selection_sort(list: &String) -> String {
    ...
    temp_list.into_iter().map(|n| n.to_string()).collect::<Vec<String>>().join(" ")
}
...
fn do_solver(){
    ...
    if let Err(e) = stdin().read_line(&mut line) {panic!("Error reading input: {}", e)}

    line = line.trim().to_owned();
    if line.eq("quit"){quit()}
...
fn print_input(unsorted: &String){
    println!("Input file had: {} \nSorted input is: {} \n", unsorted, selection_sort(&unsorted))
}
fn main() {
    ...
    loop {
        ...
        match stdin().read_line(&mut line){
            Ok(it) => it, //no sesta utilitzant
            Err(e) => panic!("{}", e),
        };
        line = line.trim().to_owned();
        
        if line.eq("solver") {do_solver()
        } else if line.eq("quit") {quit()
        } else if line.eq("input") {print_input(&input)
        } else {println!("\n{} ta puta mare crack.\n", line);}
    }
